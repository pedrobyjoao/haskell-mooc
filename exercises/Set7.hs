...
member _ (Set [])     = False
member v (Set (x:xs)) = if v == x then True else member v (Set xs)
...
add v (Set (x:xs))
  | x == v = Set (x:xs)
  | v < x = Set (v : x : xs)
  | otherwise = case add v (Set xs) of
    (Set xs') -> Set (x:xs')

-- mapSet :: (a -> b) -> Set a -> Set b
-- mapSet _ (Set []) = emptySet
-- mapSet f (Set xs) = Set (fmap f xs)
------------------------------------------------------------------------------
...
data State = Start | Error | Finished | Egged |
  EggedFloured | EggedSugared | EggedFlouredSugared | EggedFlouredSugaredMixed
  deriving (Eq,Show)
...
step Start _    = Error
step cs e = case (cs, e) of
  (Egged, AddFlour)                -> EggedFloured
  (Egged, AddSugar)                -> EggedSugared
  (Egged, _)                       -> Error

  (EggedFloured, AddSugar)         -> EggedFlouredSugared
  (EggedFloured, _)                -> Error

  (EggedSugared, AddFlour)         -> EggedFlouredSugared
  (EggedSugared, _)                -> Error

  (EggedFlouredSugared, Mix)       -> EggedFlouredSugaredMixed
  (EggedFlouredSugared, _)         -> Error

  (EggedFlouredSugaredMixed, Bake) -> Finished
  (EggedFlouredSugaredMixed, _)    -> Error
...
passwordAllowed p r = case r of
  MinimumLength n  -> length p >= n
  ContainsSome s   -> or (map (\c -> elem c p) s)
  DoesNotContain s -> not $ or (map (\c -> elem c p) s)
  And pr1 pr2      -> passwordAllowed p pr1 && passwordAllowed p pr2
  Or pr1 pr2       -> passwordAllowed p pr1 || passwordAllowed p pr2
...
data Arithmetic = Operation Op Arithmetic Arithmetic | Literal Integer
  deriving Show
...
operation op a b = Operation (operationParse op) a b
  where
    operationParse :: String -> Op
    operationParse "*" = Mult
    operationParse "+" = Add
    operationParse "-" = Sub
...
evaluate (Operation Mult a b) = evaluate a * evaluate b
evaluate (Operation Sub a b)  = evaluate a - evaluate b

render :: Arithmetic -> String
render (Literal n)          = show n
render (Operation Add a b)  = "(" ++ render a ++ "+" ++ render b ++ ")"
render (Operation Sub a b)  = "(" ++ render a ++ "-" ++ render b ++ ")"
render (Operation Mult a b) = "(" ++ render a ++ "*" ++ render b ++ ")"
