...
statements xs ys = liftA2 buildPhraseIs xs ys ++ liftA2 buildPhraseIsNot xs ys
  where buildPhraseIs x y = x ++ " is " ++ y
        buildPhraseIsNot x y = x ++ " is not " ++ y
...
calculator op n = opToFn op <*> (readMaybe n :: Maybe Int)
  where opToFn p = case p of
          "negate" -> Just negate
          "double" -> Just (*2)
          _        -> Nothing
...
validateDiv a b = liftA2 div (pure a) (check (b/=0) "Division by zero!" b)
...
validateAddress streetName streetNumber postCode =
  ...
  where
    ...
    checkCode = check isValidCode "Invalid postcode" postCode
    isValidCode = length postCode == 5 && all isDigit postCode

    checkNum = check isValidNum "Invalid street number" streetNumber
    isValidNum = length streetNumber <= 5 && all isDigit streetNumber
...
boolOrInt a =
  check isInt "Not a Bool" (Right readInt)
  <|> check isBool "Not an Int" (Left readBool)
  where isInt = Nothing /= (readMaybe a :: Maybe Int)
        isBool = Nothing /= (readMaybe a :: Maybe Bool)
        readInt = maybe 0 id (readMaybe a :: Maybe Int)
        readBool = maybe False id (readMaybe a :: Maybe Bool)
...
normalizePhone s = checkLength *> checkChars trimmedS
  where trimmedS = filter (not . isSpace) s
        checkLength = check (length trimmedS <= 10) "Too long" trimmedS

        checkChars :: String -> Validation String
        checkChars [] = pure []
        checkChars (x:xs) = liftA2 (:) checkChar (checkChars xs)
          where
            checkChar = check (isDigit x) ("Invalid character: " ++ [x]) x
...
parseExpression s = validate (words s)
  where
    validate [a, op, b] = checkOp op <*> checkArg a <*> checkArg b
      where checkArg x = checkNum x <|> checkVar x
    validate e = check False ("Invalid expression: " ++ s) (Plus (Number 0) (Number 0))

    checkNum arg =
      check (all isDigit arg) ("Invalid number: " ++ arg) (Number (read arg :: Int))
    checkVar v =
      check (all isAlpha v && length v == 1) ("Invalid variable: " ++ v) (Variable (head v))
    checkOp op =
      check (op `elem` ["+", "-"]) ("Unknown operator: " ++ op) opWrapped
        where opWrapped = if op == "+" then Plus else Minus
...
instance Functor Priced where
  fmap f (Priced n b)= Priced n (f b)

instance Applicative Priced where
  pure n = Priced 0 n
  liftA2 f (Priced v1 i1) (Priced v2 i2) = Priced (v1 + v2) (f i1 i2)
...
myFmap f x = myLiftA2 (\f' x' -> f' x') (myPure f) x
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap fn (Both a) = Both (fmap (\x -> fmap (\y -> fn y) x) a)
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  pure n = Both (pure (pure n))
  liftA2 f (Both b1) (Both b2) = Both $ liftA2 (\ga gb -> liftA2 (\a b -> f a b) ga gb) b1 b2
