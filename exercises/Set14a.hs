...
greetText n = T.pack "Hello, " <> truncated n <> T.pack "!"
  where
    truncated n' = if T.length n' > 15 then T.take 15 n' <> T.pack "..." else n'
...
shout :: T.Text -> T.Text
shout t = T.unwords $ map upperIfOdd $ zip (T.words t) [1..]
  where upperIfOdd (word, idx) = if odd idx then T.toUpper word else word
...
longestRepeat t = fst . snd $ T.foldl' f (' ', (0, 0)) t
  where
    f :: (Char, (Int, Int)) -> Char -> (Char, (Int, Int))
    f (repeating, (repeatedN, biggest)) char
      | repeating == char && repeatedN+1 > biggest = (repeating, (repeatedN+1, biggest+1))
      | repeating == char = (repeating, (repeatedN+1, biggest))
      | otherwise = (char, (1, biggest))
