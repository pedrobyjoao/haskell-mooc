...
visit maze place = do
  s <- get
  if place `elem` s then return ()
    else do modify (\s -> place : s)
...
path maze place1 place2 = place2 `elem` visitables
  where visitables = snd $ runState (visit maze place1) []
...
findSum2 ks ns = do
  ...
  k2 <- ks
  let sum = k1 + k2
  if sum `elem` ns then [(k1, k2, sum)] else []
...
-- I didn't do alone, I needed help for that :/
allSums :: [Int] -> [Int]
allSums [] = [0]
allSums (x:xs) = do
  s <- allSums xs
...
instance Functor SL where
  -- implement fmap
  fmap f (SL sf) = SL (apply . sf)
    where apply (x, state, logs) = (f x, state, logs)
...
instance Monad SL where
  ...
  (>>=) (SL sf) f = SL $ \s ->
    let
      (val1, state1, logs1) = sf s
      newSF = f val1
      (val2, state2, logs2) = runSL newSF state1
    in
      (val2, state2, logs1++logs2)
...
-- Interesting, we're basically sharing state between two different calls
