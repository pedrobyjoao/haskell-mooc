...
instance (Functor f, Foldable f, Price a) => Price (f a) where
  price fa = sum (fmap price fa)
...
instance Num RationalNumber where
  (RationalNumber a b) + (RationalNumber c d) = simplify $ RationalNumber (aM + cM) newDen
    where
      newDen = lcm b d
      aM = a * div newDen b
      cM = c * div newDen d
